...
for j = 1:length(n_dimensions)
    ...
    for m = 1:nm
        semilogy(res{m}, styles{m}, 'LineWidth', 1.2);
    end
...
print(fig1, './plots/test_n_2d.png', '-dpng');


%% -------- Plot Runtime Scaling --------
figure('name','Runtime Scaling');
...
print(fig2, './plots/test_n_2d_runtime.png', '-dpng');

%% -------------------- Timing Summary --------------------

disp("Execution times (seconds):")
disp(array2table(times,"VariableNames",methods,"RowNames",string(n_dimensions)))
% Create the table object to store the timing results to a csv file
T = array2table(times, "VariableNames", methods, "RowNames", string(n_dimensions));
% writetable(T, './csv/test_n_2d_runtime.csv');
% disp("Table saved to test_n_2d_timing_results.csv");