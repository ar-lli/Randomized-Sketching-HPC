...
#
    ...
    printf("1. Coomputing Gaussian Sketching...\n")

    # Hadamard
    S2 = hadamard(n^2) / sqrt(k1);
    S2 = S2(:, randsample(n^2, k1));
    [~, res2] = my_gmres_sketch(A, b, 1e-6, maxit, S2);
    printf("2. Coomputing Walsh-Hadamard Sketching...")

    % # Hadamard
    % # Pre-generate the randomization parameters
    % D_vec = sign(randn(n^2, 1));  % Random sign flips (Diagonal matrix D)
    % row_idx = randperm(n^2, k1);   % Random row selection (Sampling matrix R)

    % # Define the function handle
    % # We use 'hadamard' ordering for speed/consistency
    % S2 = @(x) (sqrt(n^2/k1) * subsref(fwht(k1, x .* D_vec), ...
    %             struCt('type', '()', 'subs', {{row_idx, ':'}})));
    % [~, res2] = my_gmres_sketch(A, b, 1e-6, maxit, S2);
    % printf("2. Coomputing Walsh-Hadamard Sketching...")
