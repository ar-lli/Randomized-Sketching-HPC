# This test compares performace of GMRES and sGMRES algorithm against 4 matrices of the Sparse Suite Matrix collection
...
for j = 1:length(matrices)
    ...
    # 2. Create axes with the manual position
        ...
        # Matrix definition
        A = gallery(char(matrices(j)), n^2);
        x_true = ones(n^2,1);
        b = A * x_true;

        maxit = round(n^2 * 0.02);
    ...
    # --- Subplot Rendering ---
        ...
        grid on;
        title(['Matrix = ', char(matrices(j)), ', ', 'n = ', num2str(n)]);
